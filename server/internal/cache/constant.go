...
// 滑块x坐标过期时间 n 分钟
const SLIDER_X_EXPIRATION_TIME = time.Minute * time.Duration(5)

// 人机验证状态
const CAPTCHA_STATUS_KEY = "captcha_status_key:"

// 人机验证状态过期时间 n 分钟
const CAPTCHA_STATUS_EXPIRATION_TIME = time.Minute * time.Duration(30)

// 登录尝试次数缓存标识符
const LOGIN_TRY_COUNT_KEY = "login_try_count_key:"

// 登录尝试次数过期时间 n 分钟
const LOGIN_TRY_COUNT_EXPIRATION_TIME = time.Minute * time.Duration(30)
...
// 验证token过期时间 n 分钟
const ACCESS_TOKEN_EXPIRATION_TIME = time.Minute * time.Duration(60)
...
// 上传文件过期时间 n 分钟
const UPLOAD_IMAGE_EXPIRATION_TIME = time.Minute * time.Duration(20)

// 视频分区缓存标识符
const VIDEO_PARTITION_KEY = "video_partition_key"

// 视频分区过期时间 不过期
const VIDEO_PARTITION_EXPIRATION_TIME = 0

// 文章分区缓存标识符
const ARTICLE_PARTITION_KEY = "article_partition_key"

// 文章分区过期时间 不过期
const ARTICLE_PARTITION_EXPIRATION_TIME = 0

// 视频切片状态标识符
const VIDEO_SLICE_STATUS = "video_slice_key:"

// 视频切片状态未使用过期时间 n 小时
const VIDEO_SLICE_EXPIRATION_TIME = time.Hour * time.Duration(24)

// 视频点击量限制标识符
const VIDEO_CLICKS_LIMIT_KEY = "video_clicks_limit_key:"

// 视频点击量限制过期时间  n 分钟
const VIDEO_CLICKS_LIMIT_EXPIRATION_TIME = time.Minute * time.Duration(60)

// 视频点击量标识符
const VIDEO_CLICKS_KEY = "video_clicks_key:"

// 点击量过期时间  n 小时
const VIDEO_CLICKS_EXPIRATION_TIME = time.Hour * time.Duration(24)
...
// 重置密码验证状态过期时间 n 分钟
const RESET_PWD_CHECK_EXPIRATION_TIME = time.Minute * time.Duration(30)
...
// 文章点击量限制过期时间  n 分钟
const ARTICLE_CLICKS_LIMIT_EXPIRATION_TIME = time.Minute * time.Duration(60)
