...
import { reviewPlaylistFailedAPI } from "@/api/playlist";
...
const getPositionText = (position: number) => {
  return positionOptions.find(item => item.value === position)?.label;
}

const getQuestionText = (question: number) => {
  return questionOptions.find(item => item.value === question)?.label;
}
...
const handleSubmit = async () => {
  ...
  const res = await reviewPlaylistFailedAPI({
    id: props.playlistId,
